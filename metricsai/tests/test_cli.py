...
def test_list_modules(capsys: pytest.CaptureFixture[str]) -> None:
    # Utility flag: returns before the tab requirement, so no --tab needed.
...
def test_dry_run_stub_module(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert cli.main(["--dry-run", "--module", "build_pr", "--tab", "CXT"]) == 0
...
def test_week_ending_override(capsys: pytest.CaptureFixture[str]) -> None:
    argv = ["--dry-run", "--module", "build_pr", "--tab", "CXT", "--week-ending", "2026-06-07"]
    assert cli.main(argv) == 0
    assert '"week_ending_date": "2026-06-07"' in capsys.readouterr().err


def test_unknown_module_errors() -> None:
    assert cli.main(["--dry-run", "--module", "nope", "--tab", "CXT"]) == 2
...
def test_week_ending_day_override(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    argv = ["--dry-run", "--module", "build_pr", "--tab", "CXT", "--week-ending-day", "sunday"]
    assert cli.main(argv) == 0
    assert captured["weekday"] == 6  # Sunday


def test_invalid_week_ending_day_errors() -> None:
    argv = ["--dry-run", "--module", "build_pr", "--tab", "CXT", "--week-ending-day", "funday"]
    assert cli.main(argv) == 2


def test_tab_required_for_gather(capsys: pytest.CaptureFixture[str]) -> None:
    # No --tab and no METRICSAI_WEBHOOK_TAB -> usage error before any gathering.
    assert cli.main(["--dry-run", "--module", "build_pr"]) == 2
    assert "No destination tab" in capsys.readouterr().err


def test_tab_invalid_choice_rejected_by_argparse() -> None:
    # argparse rejects an out-of-set --tab value (exits 2).
    with pytest.raises(SystemExit):
        cli.main(["--dry-run", "--module", "build_pr", "--tab", "NOPE"])


def test_tab_from_env_satisfies_requirement(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("METRICSAI_WEBHOOK_TAB", "DMOD")
    assert cli.main(["--dry-run", "--module", "build_pr"]) == 0


def test_tab_invalid_env_errors(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    # argparse only constrains --tab; an out-of-set env value is caught at the run gate.
    monkeypatch.setenv("METRICSAI_WEBHOOK_TAB", "BOGUS")
    assert cli.main(["--dry-run", "--module", "build_pr"]) == 2
    assert "Invalid tab" in capsys.readouterr().err
...
def test_security_without_repos_errors(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert cli.main(["--dry-run", "--module", "security", "--tab", "CXT"]) == 2


def test_post_without_url_errors() -> None:
    # No --dry-run and no URL configured -> usage error.
    assert cli.main(["--module", "build_pr", "--tab", "CXT"]) == 2
...
def test_post_calls_client(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    argv = ["--module", "build_pr", "--tab", "CXT", "--url", "https://example.com/exec"]
    assert cli.main(argv) == 0
...
def test_skip_sechub_flag(
    ...
    argv = ["--dry-run", "--module", "security", "--repo", "o/r", "--tab", "CXT", "--skip-sechub"]
    assert cli.main(argv) == 0
    err = capsys.readouterr().err
...
def test_all_authors_warns_when_author_also_set(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    import metricsai.modules.security as sec
    ...
    assert cli.main(argv) == 0
    assert "are ignored for the security scan" in capsys.readouterr().err


def test_all_authors_alone_does_not_warn(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    import metricsai.modules.security as sec
    ...
    assert cli.main(argv) == 0
    assert "are ignored for the security scan" not in capsys.readouterr().err
...
def test_all_modules_run_by_default(
    ...
    assert cli.main(["--dry-run", "--tab", "CXT"]) == 0
    err = capsys.readouterr().err
    assert "build_pr_num_comments" in err
