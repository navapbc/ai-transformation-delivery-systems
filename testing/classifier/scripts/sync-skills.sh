#!/usr/bin/env bash
# scripts/sync-skills.sh   (testing/classifier workstream)
#
...
#
# This is the testing-workstream sibling of the security workstream's
# scripts/sync-skills.sh; it follows the same conventions exactly, just rooted
# at the testing/classifier/ subtree and syncing the test-classifier skill.
#
...
# ── Resolve the classifier root ────────────────────────────────────────────
# Paths (.skills/, .claude/, etc.) are relative to the testing/classifier/
# subtree, NOT the repo root, so anchor on this script's location:
#   .../testing/classifier/scripts/<this file>  →  .../testing/classifier
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CLASSIFIER_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
cd "${CLASSIFIER_ROOT}"
