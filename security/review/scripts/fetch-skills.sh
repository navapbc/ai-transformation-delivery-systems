#!/usr/bin/env bash
# Vendor pr-review from navapbc/agent-skills into a gitignored .skills-vendor/
# that the dispatcher reads from. Pin to a tag/SHA (never a branch) so a
# downstream edit can't silently change CI; bump AGENT_SKILLS_REF to upgrade.
# If the ref doesn't resolve, the dispatcher falls back to .skills/.
set -euo pipefail
