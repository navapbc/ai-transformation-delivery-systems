...
`git diff --cached`; if a non-default diff range was passed, the
AI_REVIEW_AGAINST environment variable will be set to a ref, and the diff
range is `git diff $AI_REVIEW_AGAINST HEAD`, OR, when
AI_REVIEW_INCLUDE_STAGED=1 (the --unpushed mode: all committed + staged
changes not yet pushed), `git diff --cached $AI_REVIEW_AGAINST`).
